...
export function makeIndexFiles () {
  ...
  for (const lang of LANGUAGES) {
    ...
    {
      const ndjson = fs.readFileSync(path.join(__dirname, `../../public/data/${lang}/stats.ndjson`), { encoding: 'utf-8' })
      let start = 0
...

    {
      ...
      fs.writeFileSync(
        path.join(__dirname, '../../public/data', lang, 'stats-ref.index.bin'),
        indexData
      )
    }

    {
      ...
      fs.writeFileSync(
        path.join(__dirname, '../../public/data', lang, 'stats-matcher.index.bin'),
        indexData
      )
    }
  }
  ...
  for (const lang of LANGUAGES) {
    ...
    let lineStarts
    {
      const ndjson = fs.readFileSync(path.join(__dirname, `../../public/data/${lang}/items.ndjson`), { encoding: 'utf-8' })
      let start = 0
...

    {
      ...
      fs.writeFileSync(
        path.join(__dirname, '../../public/data', lang, 'items-name.index.bin'),
        indexData
      )
    }

    {
      ...
      fs.writeFileSync(
        path.join(__dirname, '../../public/data', lang, 'items-ref.index.bin'),
        indexData
      )
    }
  }
