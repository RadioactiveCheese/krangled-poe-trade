<template>
  <div class="mb-3 rounded border border-gray-700 bg-gray-900 p-2">
    <div class="mb-2 text-gray-400">{{ t('mercenary.hidden_stats') }}</div>
    <div v-if="loading" class="pt-2 text-gray-500">{{ t('mercenary.loading') }}</div>
    <div v-else-if="error" class="flex items-center gap-2 pt-2 text-red-400">
      <span>{{ t('mercenary.load_error', [error]) }}</span>
      <button type="button" class="btn" @click="loadOptions">{{ t('mercenary.retry') }}</button>
    </div>
    <template v-else>
      <div v-if="skillGroups.length" class="mb-3">
        <div class="mb-1 text-xs text-gray-500">{{ t('mercenary.available_skills', [build]) }}</div>
        <div class="max-h-56 space-y-2 overflow-y-auto pr-1">
          <div v-for="group in skillGroups" :key="group.kind">
            <div class="mb-1 text-xs uppercase text-gray-500">{{ t(`mercenary.${group.kind}`) }}</div>
            <div class="flex flex-wrap gap-1">
              <button
                v-for="stat in group.stats"
                :key="stat.id"
                type="button"
                class="rounded px-2 py-1 text-left"
                :class="isSelected(stat)
                  ? 'bg-gray-600 text-gray-100 hover:bg-red-900'
                  : 'bg-gray-800 text-gray-300 hover:bg-gray-700'"
                :aria-pressed="isSelected(stat)"
                @click="toggleStat(stat)">
                <i v-if="isSelected(stat)" class="fas fa-check mr-1 text-xs" />{{ stat.text }}
              </button>
            </div>
          </div>
        </div>
      </div>
      <div class="relative">
        <input
          v-model="query"
          class="w-full rounded bg-gray-800 px-2 py-1"
          :placeholder="t(skillGroups.length ? 'mercenary.support_placeholder' : 'mercenary.filter_placeholder')"
          @focus="openSuggestions"
          @blur="closeSuggestions"
          @keydown.enter.prevent="addFirstMatch">
        <div
          v-if="showSuggestions && query && matchingStats.length"
          class="absolute left-0 right-0 z-20 mt-1 max-h-48 overflow-y-auto rounded border border-gray-600 bg-gray-800 shadow-lg">
          <button
            v-for="stat in matchingStats"
            :key="stat.id"
            type="button"
            class="flex w-full gap-2 px-2 py-1 text-left hover:bg-gray-700"
            @mousedown.prevent="addStat(stat)">
            <span class="w-14 shrink-0 text-xs uppercase text-gray-500">{{ t(`mercenary.${stat.kind}`) }}</span>
            <span>{{ stat.text }}</span>
          </button>
        </div>
      </div>
      <div v-if="query && showSuggestions && !matchingStats.length" class="pt-2 text-gray-500">
        {{ t('mercenary.no_matches') }}
      </div>
      <div v-if="selectedTagStats.length" class="mt-2 flex flex-wrap gap-1">
        <button
          v-for="stat in selectedTagStats"
          :key="stat.tradeId[0]"
          type="button"
          class="rounded bg-gray-700 px-2 py-1 text-left text-gray-200 hover:bg-red-900"
          :title="t('mercenary.remove_filter')"
          @click="removeStat(stat)">
          <span class="mr-1 text-xs uppercase text-gray-400">{{ kindLabel(stat) }}</span>
          {{ stat.text }} <i class="fas fa-times ml-1 text-xs" />
        </button>
      </div>
    </template>
  </div>
</template>

<script lang="ts">
import { computed, defineComponent, onUnmounted, PropType, ref, shallowRef } from 'vue'
import { useI18n } from 'vue-i18n'
import { StatFilter, FilterTag } from './interfaces'
import { selectMercenaryStat } from './mercenary-picker-state'
import {
  loadMercenaryTradeData,
  MercenaryBuildSkills,
  MercenaryTradeData,
  MercenaryTradeStat
} from '../trade/mercenary-trade-data'

export default defineComponent({
  name: 'MercenaryFilters',
  props: {
    stats: {
      type: Array as PropType<StatFilter[]>,
      required: true
    },
    build: {
      type: String,
      required: true
    }
  },
  emits: ['add', 'remove'],
  setup (props, { emit }) {
    const query = ref('')
    const showSuggestions = ref(false)
    const loading = ref(true)
    const error = ref<string | null>(null)
    const availableStats = shallowRef<MercenaryTradeStat[]>([])
    const tradeData = shallowRef<MercenaryTradeData | null>(null)
    let closeSuggestionsTimer: number | undefined

    function loadOptions () {
      loading.value = true
      error.value = null
      loadMercenaryTradeData()
        .then(data => {
          tradeData.value = data
          availableStats.value = data.stats
        })
        .catch(err => { error.value = (err as Error).message })
        .finally(() => { loading.value = false })
    }
    loadOptions()

    const selectedStats = computed(() => props.stats.filter(stat =>
      stat.tag === FilterTag.MercenarySkill ||
      stat.tag === FilterTag.MercenarySupport
    ))
    const selectedIds = computed(() => new Set(selectedStats.value.map(stat => stat.tradeId[0])))
    const buildSkills = computed<MercenaryBuildSkills | undefined>(() =>
      tradeData.value?.skillsByBuild.get(props.build))
    const skillGroups = computed(() => {
      if (!buildSkills.value) return []
      return (['primary', 'secondary', 'utility'] as const).map(kind => ({
        kind,
        stats: buildSkills.value![kind]
      }))
    })
    const selectedTagStats = computed(() => buildSkills.value
      ? selectedStats.value.filter(stat => stat.tag === FilterTag.MercenarySupport)
      : selectedStats.value)

    const matchingStats = computed(() => {
      const search = query.value.trim().toLocaleLowerCase()
      if (!search) return []
      return availableStats.value
        .filter(stat =>
          (!buildSkills.value || stat.kind === 'support') &&
          !selectedIds.value.has(stat.id) &&
          stat.text.toLocaleLowerCase().includes(search))
        .slice(0, 12)
    })

    function addStat (stat: MercenaryTradeStat) {
      selectMercenaryStat(stat, filter => emit('add', filter), query, showSuggestions)
    }

    function removeStat (stat: StatFilter) {
      emit('remove', stat)
    }

    function isSelected (stat: MercenaryTradeStat) {
      return selectedIds.value.has(stat.id)
    }

    function toggleStat (stat: MercenaryTradeStat) {
      const selected = selectedStats.value.find(selected => selected.tradeId[0] === stat.id)
      if (selected) {
        removeStat(selected)
      } else {
        addStat(stat)
      }
    }

    function clearCloseSuggestionsTimer () {
      if (closeSuggestionsTimer !== undefined) {
        window.clearTimeout(closeSuggestionsTimer)
        closeSuggestionsTimer = undefined
      }
    }

    function openSuggestions () {
      clearCloseSuggestionsTimer()
      showSuggestions.value = true
    }

    function closeSuggestions () {
      clearCloseSuggestionsTimer()
      closeSuggestionsTimer = window.setTimeout(() => {
        showSuggestions.value = false
        closeSuggestionsTimer = undefined
      }, 100)
    }

    onUnmounted(clearCloseSuggestionsTimer)

    const { t } = useI18n()
    return {
      t,
      query,
      showSuggestions,
      loading,
      error,
      loadOptions,
      selectedStats,
      selectedTagStats,
      skillGroups,
      matchingStats,
      addStat,
      removeStat,
      isSelected,
      toggleStat,
      openSuggestions,
      addFirstMatch () {
        if (matchingStats.value[0]) addStat(matchingStats.value[0])
      },
      closeSuggestions,
      kindLabel (stat: StatFilter) {
        return t(stat.tag === FilterTag.MercenarySkill
          ? 'mercenary.skill'
          : 'mercenary.support')
      }
    }
  }
})
</script>
